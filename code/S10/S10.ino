...
const char* topic="S10/sen"; //change according to the device
...
#define URL_fw_Version "https://raw.githubusercontent.com/kaatrusenai/camp_a/main/S10.txt"
#define URL_fw_Bin "https://raw.githubusercontent.com/kaatrusenai/camp_b/main/S10.bin"
...
const char* WIFI_SSID = "KDONGLE10";
const char* WIFI_PASSWD = "K@@tru!810";
...
String GetTimestampValues1() // dateparser for gps
{
  ...
  mn = mn + 30;
   ...
   localstr = (gps.date.year() + delimiter3 + gps.date.month() + delimiter3 + gps.date.day() +"   "+ delimiter1 + hr + delimiter4 + mn );
  return localstr;
}
...
addOutStr(String(getPM2senvalue()));
 ...
 esp_mqtt_client_publish (client, topic,charBuf,0,0,false);
 b=millis()-a;
 tft.fillScreen(TFT_DARKCYAN);
  tft.setTextSize(2);
  tft.setCursor(40,15, 4);
  tft.setTextColor(TFT_WHITE);
  if(gps.time.hour() !=0)
  {
  tft.println(GetTimestampValues1());
  }
  tft.setCursor(20,50, 4);
  tft.println("------------------------------");
  tft.fillRect(0,90,478-1,304-15,TFT_DARKCYAN);
  tft.setTextColor(TFT_WHITE);
  tft.setCursor(30, 110, 4);tft.setTextSize(2);
  tft.println("PM_1 : "+String(PM01Value)+"ug/m3");
  tft.setCursor(30, 175, 4);tft.setTextSize(2);
  tft.println("PM_2.5 : "+String(PM2_5Value)+"ug/m3");
  tft.setCursor(30, 235, 4);tft.setTextSize(2);
  tft.println("PM_10 : "+String(PM10Value)+"ug/m3");
  tft.setCursor(0, 290, 4);
  tft.println("------------------------------");
  delay(5000);
  tft.setCursor(50, 110, 4);tft.setTextSize(2);
  tft.fillRect(0,90,478-1,304-15,TFT_DARKCYAN);
  tft.println("TEMP : "+String(aht10.readTemperature())+" C");
  tft.setCursor(80, 210, 4);tft.setTextSize(2);
  tft.println("RH : "+String(aht10.readHumidity())+"%");
  tft.setCursor(0, 290, 4);tft.setTextSize(2);
  tft.println("------------------------------");
 delay(5000-b);
